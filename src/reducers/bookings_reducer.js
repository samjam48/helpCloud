import { FETCH_DATA, BOOKINGS_MODAL_STATUS_SHOW, CHANGE_STATUS, TEXT_CHANGE } from '../actions/index'
...
export default function(state=[], action){
    ...
    switch (action.type) {
        ...
        case CHANGE_STATUS:
            ...
            for(const i in new_state){
                if(new_state[i].id == action.payload.id) {
                    new_state[i].status = action.payload.status
        ...
        case TEXT_CHANGE:
            ...
            for(const i in new_state){
                if(new_state[i].id == action.payload.id){
                    new_state[i][action.payload.field] = action.payload.text
