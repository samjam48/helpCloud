...
const BOOKING_ROOT_URL = "/api/v1/admin/bookings/"

const csrfToken = "WzNaS9YcyDMAK7XtsBPAMrODPCUKOz6n"

export function fetchData() {   // get event data from buxxhire API
    const request = axios.get(BOOKING_ROOT_URL);
    return {
        type: 'FETCH_DATA',
        payload: request
    };
...
export function changeStatus(event){
    ...
    const request = axios({
                        method: 'patch',
                        url: BOOKING_ROOT_URL + event.target.dataset.id + '/',
                        data: {
                            "status": status
                        },
    ...
    return {
        type: 'CHANGE_STATUS',
        payload: { id, status: event.target.dataset.status }
    }
}
